...
*-* **Analysis 2** Run the model using synthetic data (data derived from training data using sampling), with objective to see how the model performs in the presence of new / unseen data
...
3. The performance of the model in the prescence of unseen / new data
...
plot_std_res <- function(milestone , data) {
  ...
  ggplot( aes(x = !!sym(fit) , y = sqrt(!!sym(res))) ) +
  geom_point(aes( color =  sd2 )) + 
  scale_color_manual(values=c("#458B74" , "#B22222")) +
   ...
   # expand_limits(x = 0, y = 0) +
    labs(title = 'Homogeneity of Variance' ,  
         subtitle = 'Reference Line should be flat and horizontal',
         x = "Predicted Value" , 
         y = "$sqrt(Std. Residuals)" , caption = "RED = Outliers") +
    theme(legend.position='none',
          plot.caption = element_text(color = "#B22222", face = "italic"))
...
* There are issues with the reconciliation of the model output which need to be resolved.
