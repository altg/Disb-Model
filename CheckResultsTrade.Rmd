---
title: "Disbursement Projection Model - Trade Finance"
...
Moody's Analytics developed a Disbursement Projection Model (DM) to project future disbursement for IsDB Trade Finance Projects as part of the ERM Project. The tests done on this DM and the results are captured in this document.
...
Types of Trade Finance Projects used in our test sample

-   **Not Disbursing** - Projects approved, but yet to begin disbursing

-   **Disbursing** - Projects which have been approved, and have started disbursing, but not yet to finish disbursing
...
+----------------+-------------------+------------------------+
| Project Type   | Time to 1st Disb. | Time 1st to Last Disb. |
+================+===================+========================+
| Not Disbursing | X                 | X                      |
+----------------+-------------------+------------------------+
| Disbursing     | \-                | X                      |
...
model_input_comp <- read_csv(file = paste0(input_dir, "isdb_test_trade_comp.csv")) %>% 
  select( -...1) %>% 
  mutate( proj_type = str_replace(project_title,  '-.*' , ''))

model_output_comp <- readRDS( file = paste0(output_dir , "model_trade_output_comp.rda")) %>% 
  janitor::clean_names() %>% 
  mutate(
    proj_type = str_replace(project_title,  '-.*' , '')
  )

profiles_comp <- readRDS(file = paste0(output_dir , "full_trade_disb_profile_comp.rda")) %>% 
  janitor::clean_names()
...
model_input_comp_t <-
  model_input_comp %>%
  mutate(
    days_from_approval_to_first_disbursement_act  = time_length(date_of_first_disbursement_act - date_of_approval,   "days"),
    days_from_first_to_final_disbursement_act = time_length(date_of_last_disb_act - date_of_first_disbursement_act,   "days")
  ) %>% 
...
comp1 <- 
  ...
  mutate(
        days_from_approval_to_first_disbursement_res = days_from_approval_to_first_disbursement_act -
      days_from_approval_to_first_disbursement,
  ...
  rename( date_of_final_disbursement_act = date_of_last_disb_act )
```
...
comp_fstd <- comp1 %>% 
  ...
  mutate(
    days_from_approval_to_first_disbursement_std_res = std_res(days_from_approval_to_first_disbursement_res)
       )                                
...
c1 <- c(  "approval_to_first_disbursement" ,
         "first_to_final_disbursement")

c2 <- c( "date_of_approval" , "date_of_first_disbursement" , "date_of_final_disbursement")
...
```{r, echo=FALSE}
wrap_plots( map(c1, plot_act_pred , comp_fstd) , ncol = 2 ) + 
  plot_annotation(
  title = 'Not Disbursing Projects',
  subtitle = 'Predicted vs. Actual (days)',
  caption = paste0('Based on a sample input of ' , nrow(comp_fstd) , ' projects')
)
```
...
p1 <- plot_res(c1[1] , comp_fstd) 

p2 <- plot_std_res(c1[1] , comp_fstd) 
...
```{r echo=FALSE}
outlier_tbl( c1[1] ,  c2[1] , c2[2] , comp_fstd) %>% 
    tab_header(
...
```{r echo=FALSE}
comp_fstd %>% 
  yardstick::metrics(days_from_approval_to_first_disbursement_act, days_from_approval_to_first_disbursement ) %>%  select(-2) %>%
...
-   The Approval to First Disbursement Model has a $R^2$ of 0.56, which signifies that the model is mostly able account for a majority of the variation in the output

-   The presence of a few large outliers $( > 2 SD )$ impacts model performance

-   Other external factors (not considered here) may be affecting the time from approval to first disbursement

## Disbursing Projects

Analysis of the Predicted First to Last Disbursement times compared to actual.

```{r, echo=FALSE}
wrap_plots( map(c1[2], plot_act_pred , comp_lstd)  ) + 
  plot_annotation(
  title = 'Disbursing Projects',
  subtitle = 'Predicted vs. Actual (days)',
  caption = paste0('Based on a sample input of ' , nrow(comp_lstd) , ' projects')
)
```
...
p1 <- plot_res(c1[2] , comp_lstd) 

p2 <- plot_std_res(c1[2] , comp_lstd) 
...
```{r echo=FALSE}
outlier_tbl( c1[2] ,  c2[2] , c2[3] , comp_lstd) %>% 
    tab_header(
    title = "Model Outliers - Not Disbursing Projects",
...
-   The First to Final Disbursement Model has a $R^2$ of 0.5

-   The presence of a few large outliers $( > 2 SD )$ maybe impacting the model performance

-   Other external factors (not considered here) maybe affecting the time from first to final disbursement
...
5.  Merge with External DS - For external indicators
6.  Write to File - Save to CSV file in format accepted by the model

```{r datainput check, include=FALSE}

model_input_check <- read_csv(file = paste0(input_dir, "isdb_test_trade_check.csv")) %>% 
  select( -...1) 

model_output_check <- readRDS( file = paste0(output_dir , "model_trade_output_check.rda")) %>% 
  janitor::clean_names() 

model_output_check <- model_output_check %>% 
  inner_join( model_input_check %>%  select(where( is.character ) , -project_title, -project_title_org  ) , by = c("project_id"))


profiles_check <- readRDS(file = paste0(output_dir , "full_trade_disb_profile_check.rda")) %>% 
  janitor::clean_names()
...
map2_dfr( ctypes , c1[1]  , check_cap) %>% 
  mutate( check_type = if_else(check_type == "NDCAP" ,"Not Disbursing Cap" , "XXX" )) %>% 
  gt() %>% 
...
check_ovr <- function( ctype , mtype ) {
  ...
  mutate(
      check_ovr = abs( !!sym(days_ovr) - !!sym(days_pred) ) <= 1 
   ) %>% 
  group_by(check_type) %>% 
...
cm <- c ("first_disbursement" , "final_disbursement" )

ctypes <- c( "NDOVR" , "CDOVR" )
...
map2_dfr( ctypes , cm  , check_ovr) %>% 
  mutate( check_type = 
            case_when( check_type == "NDOVR" ~ "Not Disbursing Override",
...
model_input_sim <- read_csv(file = paste0(input_dir, "isdb_test_trade_sim.csv")) %>% 
  select( -...1) %>% 
  mutate( proj_type = str_replace(project_title,  '-.*' , ''))
 

model_output_sim <- readRDS( file = paste0(output_dir , "model_trade_output_sim.rda")) %>% 
  janitor::clean_names() %>% 
  mutate(
    proj_type = str_replace(project_title,  '-.*' , '')
  )
...
model_output_sim <- model_output_sim %>% 
  inner_join( model_input_sim %>%  select(where( is.character ) , -project_title, -proj_type, -project_title_org  ) , by = c("project_id"))


profiles_sim <- readRDS(file = paste0(output_dir , "full_trade_disb_profile_sim.rda")) %>% 
  janitor::clean_names()
...
l <- c( "sector" , "project_type" ,  "income_classification" , "rate_type" , "revolving")
...
disb_amt <- profiles_sim %>%  summarise( total_disb = sum( amount_disbursed_between_tenors , na.rm = T ) /1e6 )
...
The output of the model calculated from the 2 different methods **matches**.
...
```{r eval=FALSE, include=FALSE}
t <- model_output_sim %>% 
  select(proj_type , days_from_approval_to_first_disbursement , days_from_first_to_final_disbursement)
...
l2 <- c( "approval_to_first" ,"first_to_final" )
...
p1 <- plot_hist(data_in , data_in$days_from_approval_to_first_disbursement)
...
(p1 | p2)  + plot_annotation(
  ...
  caption = paste0('Based on a sample input of ' , nrow(data_in) , ' Approved but not Disbursing projects')
)
...
-   The disbursement profiles for the Not Disbursing projects all have the same linear 1-to-1 relationship (\$ precentage_disbursed = standard_tenor \$)

-   For Disbursing projects, the starting point of the profile is determined by the percentage already disbursed as at the evaluation date, and then it follows a similar linear relationship

-   This is in line with the trade finance disbursement projection model design as stated in the model documentation:

    > "In view of the small number of Completed projects in the Trade Finance dataset , as well as the expected shorter timeframes between first and final disbursement for Trade Finance transactions, Moody's has recommended a simplified one-to-one mapping between disbursement timing and the proportion disbursed for Trade Finance transactions."
