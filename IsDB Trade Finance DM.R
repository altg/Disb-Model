...
if (sum(!is.na(model_input$evaluation_date)) != nrow(model_input) | sum(!is.na(model_input$date_of_approval)) != nrow(model_input)) {
  ...
  app_firstdisb_reg <- fread(input = paste0(mappings_dir, "TF Approval to First Disbursement Regression Table.csv"), stringsAsFactors = F)
  first_finaldisb_reg <- fread(input = paste0(mappings_dir, "TF First to Final Disbursement Regression Table.csv"), stringsAsFactors = F)
  
  country_mapping <- fread(input = paste0(mappings_dir, "TF Country Mapping.csv"), stringsAsFactors = F)
  disb_profile_mapping <- fread(input = paste0(mappings_dir, "TF Disbursement Profile Mapping.csv"), stringsAsFactors = F, header = T)
  ...
  for (id in 1:nrow(model_input)) {     #One project at a time
    ...
    if (is.na(proj$date_of_first_disbursement_override)) {     #Override check
      if (is.na(proj$date_of_first_disbursement)) {            #Date check
        ...
        days_from_app_to_firstdisb <- ceiling(t + remaining_time)   #Calculating days
    ...
    if (is.na(proj$date_of_final_disbursement_override)) {     #Override check
      ...
      days_from_approval_to_first_disb <- ifelse(days_from_app_to_firstdisb < 1, 0,
                                                 log(days_from_app_to_firstdisb) * first_finaldisb_reg[which(first_finaldisb_reg$input == "Days from Approval to First Disbursement"),]$value)
    ...
    disb_profile_plot <- ggplot(disb_profile, aes(days_from_first_disb, perc_disbursed)) +
      ...
      ggtitle("Trade Finance", subtitle = paste0("Disbursement Profile | Project ID: ", proj$project_id)) 
  ...
  disb_profile_plot <- ggplot(disb_summ, aes(interval, amount)) +
    ...
    ggtitle("Trade Finance", subtitle = paste0("Disbursement Profile Quarterly Summary | First Evaluation Date: ", model_input$evaluation_date[1])) + 
  ...
  disb_profile_plot2 <- ggplot(disb_summ_yearly, aes(paste0(year_of_disb), amount)) +
    ...
    ggtitle("Trade Finance", subtitle = "Disbursement Profile Yearly Summary") + 
  ...
  write.csv(x = disb_summ_yearly, file = paste0(output_dir,"IsDB Trade Finance Disbursement Modelling Yearly Summary", " ", username, " ",format(time_log, "%d-%b-%Y %H.%M.%S"), ".csv"),
            na ="", row.names = F)
