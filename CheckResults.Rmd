...
ops_data <- ops_data_in %>%
  select(ops_data_ID,
         ...
         profile,
         contains("date_of")) %>%
  select( !contains("planned") ) %>% 
  mutate(across(contains("date_of") , ~ ymd(.x))) %>%
  mutate(
    days_from_approval_to_signature = time_length(date_of_signature - date_of_approval , "days"),
    days_from_signature_to_effectiveness = time_length(date_of_effective - date_of_signature, "days"),
    days_from_effectiveness_to_first_disbursement  = time_length(date_of_first_disb - date_of_effective,   "days"),
    days_from_first_to_final_disbursement = time_length(date_of_last_disb - date_of_first_disb,   "days")
  ) %>%  
  mutate(
    proj_type = case_when(
      is.na(date_of_signature) ~ "Not Signed",
      is.na(date_of_effective) ~ "Not Effective",
      is.na(date_of_first_disb) ~ "Not Disbursing",
      is.na(date_of_last_disb) ~ "Disbursing",
      T ~ "Completed"
    ),
    proj_type_code = case_when(
      is.na(date_of_signature) ~ 1,
      is.na(date_of_effective) ~ 2,
      is.na(date_of_first_disb) ~ 3,
      is.na(date_of_last_disb) ~ 4,
      T ~ 5
    )
  )
...
ops_data %>% 
  introduce() %>%  
  pivot_longer(1:9, names_to = "Item" , values_to = "value") %>% 
  gt()
```
...
t1 <- model_output %>% 
  filter( proj_type == "Not Signed") %>% 
  transmute( project_id  = project_id , days_from_approval_to_signature, type = "ModelOutput")

t2 <- ops_data %>% 
   filter( proj_type_code > 1 , 
           year(date_of_approval) > 2016, 
           !sub_mode_of_finance %in% ("Grant") ) %>% 
  transmute( project_id  = as.character(ops_data_ID) , days_from_approval_to_signature, type = "Training")

t1 %>% 
  bind_rows(t2) %>% 
  filter( days_from_approval_to_signature < 1000) %>% 
  ggplot(aes(days_from_approval_to_signature)) + 
...
```{r join with projected}

test1 <- 
  model_output %>% 
  inner_join(model_input_t , by = "project_id" )
...
test_sig %>% 
  ggplot( aes(x = days_from_approval_to_signature_act , y = days_from_approval_to_signature)) +
  geom_point() + 
...
test_eff %>% 
  ggplot( aes(x = days_from_signature_to_effectiveness_act , y = days_from_signature_to_effectiveness)) +
  geom_point() + 
  geom_abline(slope = 1 , intercept = 0) +
  geom_smooth(method = "lm") +
  expand_limits(x = 0, y = 0)
...
test_fstd %>% 
  ggplot( aes(x = days_from_effectiveness_to_first_disbursement_act, y = days_from_effectiveness_to_first_disbursement)) +
  geom_point() + 
  geom_abline(slope = 1 , intercept = 0) +
  geom_smooth(method = "lm") +
  expand_limits(x = 0, y = 0)
...
test_lstd %>% 
  ggplot( aes(x = days_from_first_to_final_disbursement_act, y = days_from_first_to_final_disbursement)) +
  geom_point() + 
  geom_abline(slope = 1 , intercept = 0) +
  geom_smooth(method = "lm") +
  expand_limits(x = 0, y = 0)
...
comp1 %>% 
  yardstick::iic(days_from_approval_to_signature_act, days_from_approval_to_signature )
