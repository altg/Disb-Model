...
model_input$date_of_signature <- ymd(model_input$date_of_signature)
model_input$date_of_signature_override <- ymd(model_input$date_of_signature_override)
model_input$date_of_effectiveness <- ymd(model_input$date_of_effectiveness)
model_input$date_of_effectiveness_override <- ymd(model_input$date_of_effectiveness_override)
...
if (sum(!is.na(model_input$evaluation_date)) != nrow(model_input) | sum(!is.na(model_input$date_of_approval)) != nrow(model_input)) {
  dlg_message("Dates are not in the correct format. Format should be yyyy-mm-dd. Please check the inputs and then run the code again.")
  ...
  for (id in 1:nrow(model_input)) {     #One project at a time
    ...
    if (is.na(proj$date_of_signature_override)) {     #Override check
      if (is.na(proj$date_of_signature)) {            #Date check
        ...
        days_from_app_to_sig <- ceiling(t + remaining_time)   #Calculating days
    ...
    if (is.na(proj$date_of_effectiveness_override)) {     #Override check
      if (is.na(proj$date_of_effectiveness)) {            #Date check
        ...
        days_from_sig_to_eff <- ceiling(t + remaining_time)   #Calculating days
    ...
    if (is.na(proj$date_of_first_disbursement_override)) {     #Override check
      if (is.na(proj$date_of_first_disbursement)) {            #Date check
        ...
        days_from_signature_to_effectiveness <- ifelse(days_from_sig_to_eff < 1, 0,
                                                       log(days_from_sig_to_eff) * eff_firstdisb_reg[which(eff_firstdisb_reg$input == "Days from Signature to Effectiveness"),]$value)
        ...
        days_from_eff_to_firstdisb <- ceiling(t + remaining_time)   #Calculating days
    ...
    if (is.na(proj$date_of_final_disbursement_override)) {     #Override check
      ...
      days_from_approval_to_signature <- ifelse(days_from_app_to_sig < 1, 0,
                                                log(days_from_app_to_sig) * first_finaldisb_reg[which(first_finaldisb_reg$input == "Days from Approval to Signature"),]$value)
      ...
      days_from_first_to_finaldisb <- ceiling(exp(xbeta) * ratio)   #Calculating days
    ...
    if (applicability == "No") {        #If Catch up/Shift isn't applicable
      ...
      disb_profile$days_from_first_disb <- ceiling(disb_profile$std_tenor * days_from_first_to_finaldisb)     #Days from First Disbursement
      ...
      if (max(disb_profile$perc_disb_catchup) > 100) {        #Check for greater than 100% disbursement
        disb_profile[which(disb_profile$perc_disb_catchup > 100),]$perc_disb_catchup <- 100
      ...
      disb_profile$profile_days <- ceiling(disb_profile$std_tenor * days_from_first_to_finaldisb)     #Days from First Disbursement according to Profile
      ...
      prof_days_act_disb <- IFNA(disb_profile$profile_days[match(act_perc_disb_eval_date, disb_profile$perc_disbursed)], 
                                 ifelse(disb_profile$perc_disbursed[1] > act_perc_disb_eval_date, 0, 
                                        ceiling((nrow(disb_profile[which(disb_profile$perc_disbursed < act_perc_disb_eval_date),])/100) * 
                                                  days_from_first_to_finaldisb)))       #Profile Days corresponding to Actual Percentage Disbursed
  ...
  # Quarterly Disbursement Summary from 1st Evaluation Date
  ...
  for (z in 1 : (length(summ_time_points) - 1)) {
    ...
    disb_summ <- rbind(disb_summ, data.frame(model_input$evaluation_date[1], interval, lb_date, ub_date, amount))
  ...
  disb_profile_plot <- ggplot(disb_summ, aes(interval, amount)) +
    ...
    labs(x = "Days from First Evaluation Date",y = "Total Amount Disbursed (USD)") +
    ggtitle("Projects and Line of Finance", subtitle = paste0("Disbursement Profile Quarterly Summary | First Evaluation Date: ", model_input$evaluation_date[1])) + 
  ...
  colnames(disb_summ) <- c("First Evaluation Date", "Days from First Evaluation Date", "Lower Bound Date of Interval", "Upper Bound Date of Interval", 
                           "Total Amount Disbursed (USD) in Time Interval")
  
  
  
  # Yearly Disbursement Summary
  
  full_disb_profile2 <- full_disb_profile
  
  full_disb_profile2$year_of_disb <- year(full_disb_profile2$date_of_disbursement)
  
  disb_summ_yearly <- plyr::ddply(full_disb_profile2, 'year_of_disb', summarise, amount = sum(amt_disb_btw_tenors))
  
  
  disb_profile_plot2 <- ggplot(disb_summ_yearly, aes(paste0(year_of_disb), amount)) +
    geom_col(fill = "skyblue4") +
    labs(x = "Year of Disbursement",y = "Total Amount Disbursed (USD)") +
    ggtitle("Projects and Line of Finance", subtitle = "Disbursement Profile Yearly Summary") + 
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
  
  print(disb_profile_plot2)
  
  fdcount = fdcount + 1
  fd[[fdcount]] = recordPlot() # write plot to list
  
  
  colnames(disb_summ_yearly) <- c("Year of Disbursement", "Total Amount Disbursed (USD) in the Year")
  ...
  write.csv(x = disb_summ, file = paste0(output_dir,"IsDB Projects & LoF Disbursement Modelling Quarterly Summary", " ", username, " ",format(time_log, "%d-%b-%Y %H.%M.%S"), ".csv"),
            na ="", row.names = F)
  
  write.csv(x = disb_summ_yearly, file = paste0(output_dir,"IsDB Projects & LoF Disbursement Modelling Yearly Summary", " ", username, " ",format(time_log, "%d-%b-%Y %H.%M.%S"), ".csv"),
            na ="", row.names = F)
